...
// dùng File.separator thay cho dấu / vì tuùy Hệ điều hành có thế là dấu / hoặc \
...
//        files.add("src/test/resources/configs/production.properties");
    ...
    public static void setValue(String key, String keyValue, String filePath) {
        Properties properties = new Properties();//Thêm dòng này nếu chỉ muốn ghi lại gias trị của biến cần set, nếu khi có dòng nay th nó lấy Properties toàn cục thì nó sẽ set cả các biến khác có trong Properties toàn cục vào
        try {
            if (file == null) {
                setDefaultFile();
